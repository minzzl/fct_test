...
impl Super5 {
    ...
    fn encode_cmde(&mut self) -> Bytes {
        ...
        info!("[encode -- !!!!!!!!!!!] odu_id: {}, addr: {}, unit_id: {}, current_page: {}, ",self.odu_id, self.address(), self.unit_id, self.current_page);
        //16진수로 page print
        info!("[encode -- !!!!!!!!!!!] page: {:#04x}", self.current_page);
        ...
        let mut idu_list = data_manager::facility::get_list_by_type(
            &FacilityType::Physical("idu".to_string()),
            ).unwrap_or_default();
            // ★ parent는 부모 Facility의 id이므로 odu_id로 비교!
            idu_list.retain(|f| f.parent == Some(self.odu_id));
            let idu_count = idu_list.len() as u8;
            info!("pdi_enabled: {}, IDU count: {}", pdi_enabled, idu_count);
        ...
        match page {
            ...
            0x20 | 0x22 | 0x23 | 0x24 => {
                ...
                if pdi_enabled && idu_count > 0 {
                    info!("[encode -- !!!!!!!!!!!] PDI enabled and IDU count: {}", idu_count);
                    self.current_page = 0x38;              // 첫 PDI 페이지로
                    self.weight = 9999;
                } else {
                    info!("[encode -- !!!!!!!!!!!] PDI disabled or IDU count is 0");
        ...
        let byte6 = checksum;
        bytes.put_u8(byte6);
        info!("[encode -- !!!!!!!!!!!] 결과적으로 보낸 page : {:#04x}", self.current_page);
    ...
    pub fn decode_super5(&mut self, bytes: Bytes) -> Result<()> {
        ...
        info!("decode ============================ odu_id: {}, addr: {}, unit_id: {}, current_page: {}, ",self.odu_id, self.address(), self.unit_id, self.current_page);
        //16진수로 page print
        info!("decode ============================ page: {:#04x}", page);
    ...
    fn decode_super5_page0234(&mut self, bytes: Bytes) -> Result<()> {
        ...
        if self.unit_id == 0 {
            ...
            // encode가 바로 읽을 수 있도록 ODU 스코프 flush
            data_table().write().flush(DeviceType::Odu, self.address(), None);
            info!(" ****************** ODU smart_plug_enable set: {}", smart_plug_enable);
         }
