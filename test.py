...
# 로그 파일 설정
log_file_path = config.get('global', {}).get('log_file_path', '/lg_rw/fct_test/fct_test.log')
usb_log_file_path = f"fct_test_{eth_mac}.log"
    ...
    if config[item]['enable']:
        ...
        for i in range(repeat):
            ...
            if item == 'UART':
                ...
                # 1과 2 중 랜덤하게 둘 중 하나를 선택하도록 
                wifi_options = [
                    {'ssid': 'next_test',   'address': '192.168.2.254'},
                    {'ssid': 'next_test_2', 'address': '192.168.3.254'}
                ]
                choice = random.choice(wifi_options)
                # args는 get_test_args()에서 생성된 기본 args
                args['ssid'] = choice['ssid']
                args['address'] = choice['address']
            ...
            #     args_str = f"{args['mac']} {args['min']} {args['max']}"
                ...
                com_flag = config[item].get('485', False)
                if com_flag:
                    args_str += " --485-test"
            else:
                args_str = ''
...
여기서 1과 2중에 선택이 안되는 경우도 있는걸까? 

Test started at: 2025-08-07 17:43:20
WIFI       |   FAIL   | No output


테스트 결과가 이렇게 나올 떄가 있었어 ..
...
def save_failure_logs(context, ssid, log_content):
    global current_time
...
def run_normalmode_and_capture_logs(log_dir):
    ...
    while time.time() - start_time < 20:
        ...
        try:
            ...
            data = {"error": result1.stdout}
 
        scan_log += json.dumps(data, indent=2) + "\n"
        time.sleep(5)
 
    with open(os.path.join(log_dir, "check_findnetwork_after_run_normalmode.log"), "w") as f1:
        f1.write(scan_log)
