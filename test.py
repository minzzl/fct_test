...
def sync_test_files(ip, user, remote_dir="/lg_rw/fct_test", pattern="expansion_fct_test_", pwd="allnewb2b^^"):
   log("[...] Sync test files via SSH")
   try:
       dest_dir = BASE_DIR / "synced_test_files"
       dest_dir.mkdir(exist_ok=True)
       ssh = paramiko.SSHClient()
       ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
       ssh.connect(hostname=ip, username=user, password=pwd)
       # 1. 파일 목록 가져오기
       cmd = f"ls {remote_dir}/{pattern}* 2>/dev/null"
       stdin, stdout, _ = ssh.exec_command(cmd)
       remote_files = stdout.read().decode().strip().splitlines()
       if not remote_files:
           log("[!] No matching test files found.")
           ssh.close()
           return
       for remote_path in remote_files:
           ...
           remote_data = stdout.read()
           if not remote_data:
               log(f"[x] Empty or missing: {remote_path}")
               continue
           # 3. 로컬 파일과 비교
           if local_path.exists():
               with open(local_path, "rb") as f:
                   if f.read() == remote_data:
                    #    log(f"[=] Skipped (same): {filename}")
                       continue
                   else:
                       log(f"[~] Overwriting changed file: {filename}")
           else:
               log(f"[+] Downloading new file: {filename}")
           # 4. 저장
           with open(local_path, "wb") as f:
               f.write(remote_data)
               log(f"[v] Saved: {filename}")
       ssh.close()
   except Exception as e:
       log(f"[x] File sync error: {e}")
# ────────── 기본 텍스트 입력 다이얼로그 ───────────────
...
def run_fct(host_ip:str, expansion_count: int):
   ...
   try:
       ...
       sync_test_files(host_ip, username)
...
이 코드에서 보드 접속해서 파일 복사하는 것만 가져오고 싶어. 
나는 /lg_rw 영역의 파일을 모두 복사해오고 싶어. 내가 파일을 실행시키는 위치로 
