...
이거를 각각 실행 시킬 때 발생하는 응답들을 함께 파일에 저장하고 싶어. 그리고 def reload_driver 를 실행했는데도, 동일하게 이슈가 나면 /usr/etc/normalmode.sh 을 실행시키도록 해줘 그리고 /usr/etc/normalmode.sh 를 실행 시킨 후에 find network 할때 오류가 난다면 그 응답도 별도의 파일로 저장될 수 있도록 해줘
