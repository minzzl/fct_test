   ...
   if os.path.exists(path):
       with open(path) as f:
           if any(ip in l for l in f):
               subprocess.run("ssh-keygen -R {}".format(ip), shell=True)
   return True
...
def write_cfg_to_board(mac, sn, ip, user, pwd="allnewb2b^^"):
   ...
   try:
       ...
       tmp = BASE_DIR / "_tmp_cfg.yml"                       # 임시 파일
       ...
       c.close(); tmp.unlink(missing_ok=True)
       log("[v] cfg OK"); return True
   except Exception as e:
...
def write_pc_launcher_to_board(ip, user, pwd="allnewb2b^^"):
   ...
   try:
       ...
       for local, remote in [("index.html", "index.html"), ("launcher.exe", "launcher.exe")]:
           with open(BASE_DIR / local, "rb") as f:           # BASE_DIR 추가
...
def run_fct(host_ip: str, spec_mode: bool):
   ...
   try:
       ...
       if not spec_mode:
           ...
           if not update_check_column() or not write_pc_launcher_to_board(host_ip, username): return
       else:
...
lbl_title=ttk.Label(hdr, text="ACP-i FCT", foreground=LG_RED,
