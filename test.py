...
def provision_key_after_fct(ip, user, pwd="allnewb2b^^"):
   ...
   try:
       ...
       # 1) 키 읽기 (없어도 에러나지 않게 처리)
       _, stdout, _ = c.exec_command("cat /lg_rw/etc/key 2>/dev/null || true")
       raw = stdout.read().decode("utf-8", errors="ignore")
       key = raw.rstrip("\r\n")                # sysfs에 개행 없이 정확히 쓰기
       if not key:
           log("[x] Key not found or empty: /lg_rw/etc/key")
           c.close()
           return False
       # 2) sysfs에 쓰기 (개행 안 붙임)
       #    shell redirection 대신 stdin으로 정확한 바이트만 전달
       stdin, _, stderr = c.exec_command("cat > /sys/devices/platform/kfuse_device/key_value")
       stdin.write(key)
       stdin.flush()
       stdin.channel.shutdown_write()          # 입력 종료 통지
       time.sleep(0.2)                         # 드라이버 적용 여유
       # (선택) 에러 메시지 확인
       err = stderr.read().decode().strip()
       if err:
           log(f"[x] sysfs write err: {err}")
           c.close()
           return False
       # 3) 원본 삭제
       c.exec_command("rm -f /lg_rw/etc/key")
       c.close()
       log("[v] Device key applied & cleaned")
       return True
...
def run_fct(host_ip:str, spec_mode:bool):
    ...
    try:
        ...
        if abort_event.is_set(): return
        ok = start_fct_test(host_ip, username, ask_input)
        # ✅ FCT가 성공(status==0)일 때만 키 적용 후 원본 삭제
        if ok and not abort_event.is_set():
            provision_key_after_fct(host_ip, username)
    finally:
